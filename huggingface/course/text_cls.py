...
def main():
    ...
    # list of lists of actual 選的 group label in validation

    actual_train = [ _["course_id"].split(" ") if _["course_id"] else [] for _ in rawest_ds["train"]]
    actual_valid = [ _["course_id"].split(" ") if _["course_id"] else [] for _ in rawest_ds["validation"]]
    ...
    group_df = pd.read_csv(args.data_dir + "courses.csv").fillna("").set_index("course_id")
    id_to_label = dict()
    label_to_id = dict()
    label_list = list()
    for k, v in enumerate(group_df.index):
        id_to_label[k] = v
        label_to_id[v] = k
        label_list.append(v)
    ...
    def raw2user(e):
        uid = e["user_id"]
        u = {"sentence": "", "label": e["course_id"]}
        # for col in user_df.columns:
        #     if col != "interests":
        u["sentence"] += str(user_dict[uid]["interests"])
    ...
    def preprocess_function(examples):
        ...
        for i, labels in enumerate(examples["label"]):
            ...
            for l in labels.split(" "):
                labels_matrix[i, label_to_id[l]] = 1.
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for i, preds in enumerate(predictions):
                ...
                for j, pred in enumerate(preds):
                    new_preds.append( id_to_label[pred] )
                    if j >= 50:
                        break
                predictions[i] = new_preds
            predicted += predictions # + 1 to move idx back
            loss = outputs.loss
        ...
        if not args.pred:
            with open(log_path, "a") as f:
                f.write(f"{train_loss/len(rawest_ds['train']):.7}, {mapk(actual_train, predicted, 50):.7f},")
